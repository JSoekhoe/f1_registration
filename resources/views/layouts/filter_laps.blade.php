<div class="hidden sm:flex sm:items-center sm:ms-6">
    <x-dropdown align="right" width="48">
        ...
        <x-slot name="content">
            <a href="{{ route('laps.index', ['sort' => 'placement', 'direction' => ($currentSort === 'placement' && $currentDirection === 'asc') ? 'desc' : 'asc']) }}" class="block px-4 py-2 text-sm leading-5 text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 hover:text-gray-900 dark:hover:text-gray-400 focus:bg-gray-100 dark:focus:bg-gray-700 focus:text-gray-900 dark:focus:text-gray-400">{{ __('Placement') }} ({{ $currentSort === 'placement' ? ($currentDirection === 'asc' ? 'Asc' : 'Desc') : 'Asc' }})</a>
